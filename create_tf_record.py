...
def get_example_nums(tf_records_filename):
    """
    统计tf_records图像的个数(example)个数
    :param tf_records_filename: tf_records文件路径
    :return:
    """
    nums = 0
...
def read_records(filename, resize_height, resize_width, type=None):
    ...
    elif type == 'normalization':  # 若需要归一化请使用
...
def batch_test(record_file, resize_height, resize_width):
    ...
    # 读取record函数
    tf_image, tf_label = read_records(record_file, resize_height, resize_width, type='normalization')
    image_batch, label_batch = get_batch_images(tf_image, tf_label, batch_size=4,
                                                labels_nums=5, one_hot=False, shuffle=False)
    ...
    with tf.Session() as sess:  # 开始一个会话
        ...
        for i in range(4):
            ...
            print('shape:{}, type:{}, labels:{}'.format(images.shape, images.dtype, labels))
