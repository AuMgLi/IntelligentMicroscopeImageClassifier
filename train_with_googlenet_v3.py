...
def net_evaluation(sess, loss, accuracy, val_images_batch, val_labels_batch, val_nums, merged):
    ...
    for _ in range(val_max_steps):
        ...
        val_loss, val_acc, summary = sess.run([loss, accuracy, merged],
                                              feed_dict={input_images: val_x, input_labels: val_y,
                                                         keep_prob: 1.0, is_training: False})
        val_losses.append(val_loss)
        val_accs.append(val_acc)
    mean_loss = np.array(val_losses, dtype=np.float32).mean()
    tf.summary.scalar('test_loss', mean_loss)
    mean_acc = np.array(val_accs, dtype=np.float32).mean()
    return mean_loss, mean_acc, summary
...
def step_train(train_op, loss, accuracy,
    ...
    with tf.Session() as sess:
        ...
        train_writer = tf.summary.FileWriter('data/gn_logs/train/', sess.graph)
        test_writer = tf.summary.FileWriter('data/gn_logs/validation/')
        for i in range(max_steps + 1):
            batch_input_images, batch_input_labels = sess.run([train_images_batch, train_labels_batch])
            _, train_loss, summary = sess.run([train_op, loss, merged], feed_dict={input_images: batch_input_images,
            ...
            # val测试(测试全部val数据)
            if i % val_log_step == 0:
                mean_loss, mean_acc, summary = net_evaluation(sess, loss, accuracy,
                                                     val_images_batch, val_labels_batch, val_nums, merged)
...
def train(train_record_file,
    ...
    # # train_tensor = optimizer.minimize(loss, g lobal_step)
...
if __name__ == '__main__':
    train_record_file = 'data/tfrecords/train.tfrecord'
    val_record_file = 'data/tfrecords/validation.tfrecord'
